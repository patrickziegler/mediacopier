...
void FileOperationMove::visit(const FileInfoImage& file)
{
    copyFile(file);
    fs::remove(file.path());
}

void FileOperationMove::visit(const FileInfoImageJpeg& file)
{
    copyFile(file);
    fs::remove(file.path());
}

void FileOperationMove::visit(const FileInfoVideo& file)
{
    copyFile(file);
    fs::remove(file.path());
}
